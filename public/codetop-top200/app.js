...
/* ---------- 标签 ---------- */
/* Hot 100 题号：按标题开头的 LeetCode 题号自动打标 */
const HOT100_NUMBERS = new Set([1,2,3,4,5,10,11,15,17,19,20,21,22,23,31,32,33,34,39,42,46,48,49,53,55,56,62,64,70,72,75,76,78,79,84,94,96,98,101,102,104,105,114,121,124,128,136,139,141,142,146,148,152,155,160,169,198,200,206,207,208,215,221,226,234,236,238,239,240,253,279,283,287,297,300,301,309,312,322,337,338,347,394,399,406,416,437,438,448,461,494,538,543,560,581,617,647,739,763,994]);
const TITLE_NUM_RE = /^(\d+)\s*\./;

for (const it of ITEMS) {
  const m = TITLE_NUM_RE.exec(it.title);
  if (m && HOT100_NUMBERS.has(Number(m[1]))) it.tags = ['hot100'];
}

const ITEM_BY_RANK = new Map(ITEMS.map(function (it) { return [it.rank, it]; }));
...
function renderList(filter) {
  ...
  for (const it of ITEMS) {
    const tags = it.tags || [];
    const hay = (String(it.rank) + ' ' + it.title + ' ' + it.algo + ' ' + tags.join(' ')).toLowerCase();
    const matches = !q || hay.includes(q);
    items.push({ key: 'q-' + it.rank, badge: it.rank, label: it.title, diffCls: it.diffCls, tags, matches });
  }
  let visible = 0;
  for (const it of items) {
    ...
    if (it.tags && it.tags.length) {
      ...
      chip.textContent = it.tags.join(' · ');
      btn.insertBefore(chip, btn.querySelector('.diff-dot'));
    }
...
function select(key) {
  ...
  const item = ITEM_BY_RANK.get(parseInt(String(key).slice(2), 10));
  const meta = wrap.querySelector('.q-meta');
  if (item && item.tags && meta) {
    const link = meta.querySelector('.pill.link');
    for (const tag of item.tags) {
      const pill = document.createElement('span');
      pill.className = 'pill tag';
      pill.textContent = '#' + tag;
      meta.insertBefore(pill, link || null);
    }
  }
